...
| novel-character-extractor | 原著角色提取：从用户提供的原著文本（epub/txt/文件夹，小说、轻小说、同人均可）提取指定角色的结构化人物档案（外貌/性格/爱好/说话风格/背景/三观/名场面引文），输出为 md 文件；铁律是省 token 优先——脚本能做的不用模型读，能读片段的不读整卷 |
